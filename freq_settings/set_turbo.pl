#!/usr/bin/perl
# sshaw@sgi.com copywrite sgi 2015
use strict;
...
sub Toggle_MSR{   # short term fix for turbo bug in bios
   ...
   my $FlipTheBit = 0;
   my $host=qx(hostname);
   my $tmp_a="0x" . qx(/lustre/sgi/abaqus/cases/freq_settings/rdmsr -p 0 0x199);
...
$t1 = time() if $DEBUG;
   ...
   if ( $FlipTheBit == 1 ) {
      ...
      for (my $coreid=0; $coreid < $ncores; $coreid++) {
        qx(/lustre/sgi/abaqus/cases/freq_settings/wrmsr -p $coreid 0x199 $tmp_b);
